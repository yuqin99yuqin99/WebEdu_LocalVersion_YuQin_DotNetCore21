...
//已不支持，暂时注释了，可能没弹出菜单的内容了！！！代解决！！！window.document.getElementById("sDownload").startDownload(window.sURLHeader+"common/popupTitle.htm",fnWritePopupContent);//不知道为什么直接调用parent.sURLHeader出错！必须固定initial.asp或initial.asp（既主界面文档）的放置位置，从而获得整个应用的URL根，其它文档中全部使用相对URL，便于维护和移植。
...
var iMaxLength=0;
var sString="";
//var oDiv=window.oPopup.document.getElementById("popupDiv");//由于window.createPopup()改写，此处也因此改写。
var oDiv=document.getElementById("popupDiv");//由于window.createPopup()改写，此处也因此改写。
