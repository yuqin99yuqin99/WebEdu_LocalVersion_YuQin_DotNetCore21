...
if(sDriveType!=2){
	...
	try{//如果已复制部分文件,重新复制时,有时无法删除旧文件,出错时提示
		...
		if(oFSO.folderExists(sPath+"webCourse\\lessons\\content\\webCourseCommon\\")){
			;
		}
		else{
			oFSO.createFolder(sPath+"webCourse\\lessons\\content\\webCourseCommon\\");
		}
		...
		var oFolder_webCourseCommon=oFSO.getFolder(sPathSource+"webCourse\\lessons\\content\\webCourseCommon\\");
		...
		oFSO.CopyFolder(oFolder_webCourseCommon,sPath+"webCourse\\lessons\\content\\",true);
