...
<script language="jscript">
function fnPlayClip(){
	//document.getElementById("playWindow").location.reload();
	open("flashPlayerReplay.asp","playWindow",false);
	//document.getElementById("playWindow").src=sSrc;
 }
</script>
...
try{
	var sNodeValue=0;
	var cLi=parent.opener.document.getElementsByTagName("LI");
	var intLi=cLi.length;
	var intTarget=-1;;

	for(var i=0;i<intLi;i++){
	if(cLi[i].getElementsByTagName("SPAN").item(1).style.color==parent.opener.sColorClicked){
		intTarget=i;
		break;
		}
		}
	
	if(parent.opener.sCurrentLanguageForPlayAll=""){//国语状态
			...
			for(var j=intTarget+1;j<intLi;j++){
				if(cLi[j].getElementsByTagName("A").item(0).attributes.getNamedItem("play").nodeValue!=""){
					if(j==0){
						...
						if(oTemp.childNodes[0].src.lastIndexOf("plus.gif")>0&&oTemp.getElementsByTagName("SPAN").item(1).style.color!=parent.opener.sColorClicked){
					...
					for(var k=0;k<intLi;k++){
						if(cLi[k].getElementsByTagName("A").item(0).attributes.getNamedItem("play").nodeValue!=""){
							if(k==0){
								...
								cLi[k].getElementsByTagName("SPAN").item(1).childNodes(0).nodeValue=cLi[j].getElementsByTagName("A").item(0).attributes.getNamedItem("sN").nodeValue;
