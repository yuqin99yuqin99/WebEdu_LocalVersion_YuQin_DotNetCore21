<%
 ...
 sDownFilePath = Request("sDownFilePath") '部分保存路径
 'sImagePath = sPathToClient(sImagePath,"\") '不用转,但保留在此
  
 'Response.write "<script>alert('"+sDownFilePath+"')</script>"
...
<form method="POST" id="form1" action="upfile.asp?sDownFilePath=<%=sDownFilePath%>" >
	<p><input  type="file" name="ImageName" size="28"></p>
...
<% 
  '提交后把它拷贝到指定目录下,并返回在线编辑的值
   ...
   If Not Request("ImageName")="" Then
      ...
      sFilePath = Server.mapPath("../../../"+sDownFilePath)
      ...
      Response.write "<script>parent.fnDownloadFile('" + sRndFileName + "');</script>"
   End If
