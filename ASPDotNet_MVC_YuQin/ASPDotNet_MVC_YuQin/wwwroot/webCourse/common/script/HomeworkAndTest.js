function fnHomeworkAndTest() {
   ...
   var iProblemNum = iTrLenth / 8;
    ...
    for (i = 0; i < iTrLenth; i++) {
       ...
       if ((i + 8) % 8 === 6) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" onclick=\"fnToggoleAnswerExplainDisplay();\" value=\"查看答案的解释【建议基于答案解释的具体需求，超链接相应的字符（例如.docx文件、.pptx文件）-图像视频（例如.mp4文件）-2D（例如.svg文件）-3D（例如.x3d文件）】\" /></span>" + "<span style=\"display:none\" id=\"idExplain" + sTimeStamp + Math.floor(((i + 8) / 8)) + "\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>"; }
