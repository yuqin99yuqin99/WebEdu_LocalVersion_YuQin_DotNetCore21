...
{
    public class UploadTeachingEngineeringController : Controller
    {
        public UploadTeachingEngineeringController(IHostingEnvironment hostingEnvironment)//通过注入 IHostingEnvironment 服务对象来取得Web根目录和内容根目录的物理路径
        {
        ...
        /**public void Post([FromBody] string value)
{
        ...
        public async Task<IActionResult> Post()//ASP.NET MVC 操作支持使用简单的模型绑定（针对较小文件）或流式处理（针对较大文件）上传一个或多个文件。(在此选择流式处理）)
        {
            ...
            if (new LocalVersionOrServerVersion().IsLocalVersion(this.Request.Host.ToUriComponent())) 
            {
                ...
                String filePathPartTemp = "\\webCourse\\lessons\\content\\TeachingEngineering\\";
                String filePathAll = "";
