	...
	for(var i=1;i<iParaCount;i++){
		...
		try{
			...
			var oDocument=wordApp.Documents.Add();
		 	...
		 	oDocument.Paragraphs.item(1).range.text=sNewListString+oDocument.Paragraphs.item(1).range.text;//oDocument.Paragraphs.item(1).range.listFormat.ListString 只可读!
		 	
		 		//var wordDocument= wordApp.documents.open(sCurrentPath+"/book.doc") //可能改变;
		     oDocument.WebOptions.RelyOnCSS =true;
		     oDocument.WebOptions.OptimizeForBrowser =true;//为浏览器优化
		     ...
		     oDocument.WebOptions.Encoding = 20127;//us-ascii编码
			...
			WScript.echo(cPara.item(i).range.listFormat.ListString+cPara.item(i).range.text+"发现错误，请查询错误，但仍将继续运行！");
			continue;
		}
		}
	}
